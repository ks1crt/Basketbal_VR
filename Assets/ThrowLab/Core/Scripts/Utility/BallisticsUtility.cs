...
public static class BallisticsUtility
{
    ...
    public static int SolveQuartic(double c0, double c1, double c2, double c3, double c4, out double s0, out double s1, out double s2, out double s3)
    {
        ...
        if (IsZero(r))
        {
            ...
            num = BallisticsUtility.SolveCubic(coeffs[0], coeffs[1], coeffs[2], coeffs[3], out s0, out s1, out s2);
        }
        else
        {
            ...
            num = BallisticsUtility.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);

            coeffs[2] = z + u;
            coeffs[1] = q < 0 ? v : -v;
            coeffs[0] = 1;

            if (num == 0) num += BallisticsUtility.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);
            if (num == 1) num += BallisticsUtility.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s1, out s2);
            if (num == 2) num += BallisticsUtility.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s2, out s3);
        }
    ...
    public static bool solve_ballistic_arc_lateral(Vector3 proj_pos, float lateral_speed, Vector3 target, Vector3 target_velocity, float max_height_offset, out Vector3 fire_velocity, out float gravity, out Vector3 impact_point)
    {
        ...
        double t0, t1;
        int n = BallisticsUtility.SolveQuadric(c0, c1, c2, out t0, out t1);
